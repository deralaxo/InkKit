...
type Props<V> = {
  ...
  /**
   * Funtion that returns props for the wrapper of each item.
   */
  wrapperProps?: (props: {item: Item<V>, index: number, isSelected: boolean}) => BoxProps;
};
...
function SelectInput<V>({
  ...
  wrapperProps
	...
	return (
        ...
        const boxProps: BoxProps = wrapperProps?.({item, index, isSelected}) ?? {};

				return (
					// @ts-expect-error - `key` can't be optional but `item.value` is generic T
					<Box key={item.key ?? item.value} {...boxProps} >
